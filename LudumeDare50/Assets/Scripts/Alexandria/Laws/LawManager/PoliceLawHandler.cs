        ...
        private void ApplyEffect(StoragesKeeper storagesKeeper, PoliceLawSO effect, float duration, bool reverse)
        {
            ...
            switch(effect.Type)
            {
                ...
                case PoliceLawType.Speed:
                    if(effect.InPercent) 
                        foreach(var avtozak in _avtozaks)
                            avtozak.Upgrade(avtozak.Health, value * avtozak.Speed, avtozak.Capacity, avtozak.ArrestDelay);
                    else
                        foreach(var avtozak in _avtozaks)
                            avtozak.Upgrade(avtozak.Health, value + avtozak.Speed, avtozak.Capacity, avtozak.ArrestDelay); 
                break;
                ...
                case PoliceLawType.ArrestDelay:
                    if(effect.InPercent) 
                        foreach(var avtozak in _avtozaks)
                            avtozak.Upgrade(avtozak.Health, avtozak.Speed, avtozak.Capacity, value * avtozak.ArrestDelay);
                    else
                        foreach(var avtozak in _avtozaks)
                            avtozak.Upgrade(avtozak.Health, avtozak.Speed, avtozak.Capacity, value + avtozak.ArrestDelay);
                break;
